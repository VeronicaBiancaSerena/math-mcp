...
当 1/2 路径代入后差不为 0 时，**反例搜索必须尊重全部约束（含不等式）**：在 reduced 变量（参数或消元后剩余变量）网格上，把每个点经 substitutions 映射回原变量，仅当该原变量点满足全部约束才接受为反例；否则不得判 `disproved`。因为参数化对等式约束恒成立，不等式才是绑定检查——这避免了"参数取值落在不等式可行域外却被当成反例"的不健全反证（例如在 `x>=0` 上验证 `x == |x|`，参数 `t<0` 映射到 `x<0` 不可行，应判 evidence 而非 disproved）。
...
- `method="symbolic_lagrange"`：仅等式约束。求解 `∇f = Σλ∇g` 与 `g=0`，返回实候选临界点、目标值、候选清单；`certainty="evidence"`，`method="symbolic"`，backend `sympy`。每个候选点用**约化（投影）Hessian** 二阶判定：以 `Z` 为约束 Jacobian 零空间的基，`Zᵀ H_L Z` 正定→`local_min`、负定→`local_max`、不定→`saddle`、无法判定→`indeterminate`，写入候选与 `result.optimum_point_type`；选优先取与 `goal` 匹配类型的点，否则回退到目标值最优者。
...
### 24.10 运行期 payload_schema 闸门

`math_capabilities` 暴露的 `payload_schema` 必须是**运行期真闸门**，而不仅是给 agent 看的提示。dispatcher 在解析 operation、检查 disabled 之后、规范化约束之前，用 `schema_check.validate_payload` 校验 payload：类型、`enum`、数值上下界、字符串/数组长度不符立即返回 `status="invalid_input"` 并列出前若干条错误，`backend="none"`——子进程类 operation 的坏 payload 因此连 sandbox 都不会启动。

两个运行期放宽（与 conformance oracle 的严格模式区分）：

- `reject_additional=False`：不因未知键报错。handler 本就忽略多余字段，且 §24.5 的 `payload.domains` 错位提示依赖该字段能穿透到 handler。
- `enforce_required=False`：`required` 留给 handler 校验。原因是多个 operation 接受 `expression` **或** `expr_ast`（扁平 schema 子集无法表达这种"二选一"），handler 用 `load_expression`/`require` 给出 operation 专属错误；闸门只负责类型/枚举/边界这类 handler 故意放宽的维度。

conformance oracle 仍以默认严格模式（`reject_additional=True`、`enforce_required=True`）校验每个 `example_payload`，保证 capabilities 广告的 schema 自洽。
