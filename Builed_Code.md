...
> 最近一次审计与同步：**2026-05-30**（第二轮严格审计：Z3 sort/domain 冲突、复数分支约定、
> output_too_large、worker 二进制输入加固，并补齐 §15 测试；详见 §3.7）。
...
| 测试 | **323 passed**（初始 280；第一轮审计+solve_recurrence+Tier-C 累计 +37；第二轮审计 +6） |
