...
当前（审计修复后）状态。若按**初始交付**（审计前）评估约为 **87 / 100（B+）**，主要差距来自指南 §15 明确要求
但缺失的若干测试类别，以及 4 处潜在缺陷（详见「审计前后对比」）。
...
| 4. 测试与质量保障 | 15 | 14 | 303 测试、conformance 自动生成、真实沙箱验收；初始缺 §15 多类测试（已补） |
...
| 8. 工程化与可维护性 | 5 | 4.5 | conda/pyproject、ruff+mypy strict 全绿、Linux-only 明确 |
| **合计** | **100** | **94** | |
...
- **扣 1 分**：13 个 operation 仍为 `experimental`（ode 类 4 个、probability 分布类 5 个、groebner/数值优化/数值分解/
  conic）——指南允许此发布节奏，但从「整体落地完成度」看，默认对 agent 可见的是 84/97。
...
### 4. 测试与质量保障 — 14/15
- 覆盖面广：303 测试，含 conformance（从 registry 自动生成）、capabilities、registry、error code、seed 决定性、
  **真实 bwrap 沙箱验收**、安全/Hypothesis fuzz、golden、eval、真实 stdio smoke。
- **扣 1 分（且这是审计前的主要欠账）**：指南 §15 明确点名却**初始缺失**的测试类别 ——§15.5 差分测试**整类缺失**、
  §15.4 `det(A*B)=det(A)det(B)` 与深嵌套 fuzz、§15.6 `check_identity→numeric_evidence_only`、§15.7 Z3
  `unknown`/timeout、§15.3.1 超大 stdout、§15.15 benchmark `subprocess_overhead` ——均在本轮补齐。
  这反映初始「测试完备性」与指南要求有差距（虽然总量已不少）。
...
- `environment.yml`/`pyproject.toml` 与 §7/§8 完全一致；`ruff check .`、`mypy src`（strict）全绿；Linux-only 边界
  明确（非 Linux 拒启）。
- **扣 0.5 分**：目录下 `.git` 为空目录、非有效仓库，缺版本历史/提交记录，不利于追溯与协作。
...
## 不足与改进点
1. **`experimental` 比例**：13/97 仍未晋级（尤以 ode 类、概率分布类为主），默认不可见；建议按 §10.5 逐个补
   golden/eval/差分测试后晋级。
...
5. **缺版本控制**：建议 `git init` 并纳入历史，提交本轮审计与修复。
...
| --- | --- | --- |
| 测试数 | 280 | 303 |
| implemented / experimental | 83 / 14 | 84 / 13 |
...
**结论：94/100（A）。** 这是一个工程质量优秀、边界清晰、可审计的本地数学推理 MCP；扣分集中在「初始测试完备性
欠账」「少数潜在缺陷」「experimental 比例」与「缺版本控制」，其中前两项已在本轮审计中基本修复。
